...
$sql1 = "SELECT c_id, c_name, c_email, c_phn FROM customer  WHERE c_act=0 ORDER BY c_name DESC" ;
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  for($i = 0; $i < count($food); $i++){
    ...
    if(isset($_POST[$sett])){
      ...
      $sql = "UPDATE customer SET c_act=1 WHERE c_id = '$c_id' ";
      $result = $conn->query($sql) ;
      if($result){
        header("location: 4_admin_banned.php");
      }
      else{
        echo "<script> alert(\"Couldn't remove the bann at the moment.\") </script> ";
      }
    }
  }
}
...
<ul class="general admin_menu" >
  ...
  <li><a href="4_admin_banned.php" class="general_active_menu_opt active_adm">Banned Acc</a></li>
  ...
  <hr><h3 style="color:black;">Users List</h3> <hr>
    ...
    <tr>
     ...
     <td> <form action= "<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
             <button style="width: 50%;" value="<?php echo $cid ;?>" name="bann<?php echo $i ; ?>">Lift Bann</button>
       </form> </td>
   </tr>
    <?php } ?>
  </table> <br>
